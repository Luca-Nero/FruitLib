...
- **Selection fires before the slot's item exists in the world** (it spawns ~250ms later). If you
  ...
  from `OnUpdate` until it appears. `SlotSandbox` in `2_GunsGunsGunsRefractored/SlotSandbox.cs` is
  a worked example: it attaches a primitive model and wires LMB/RMB to custom actions.
