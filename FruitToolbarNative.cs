...
namespace FruitLib
{
    ...
    public static class FruitToolbar
    {
        ...
        private static bfc _sfx;

        public static bfc Sfx
        {
            get
            {
                if (_sfx != null) return _sfx;

                var view  = _view != null ? _view : UnityEngine.Object.FindObjectOfType<ToolbarView>(true);
                var slots = view != null ? view.m_slotsViews : null;
                if (slots == null) return null;

                for (int i = 0; i < slots.Length && _sfx == null; i++)
                    if (slots[i] != null) _sfx = slots[i].ptc;
        ...
        private static void CacheSlotDrawData(int idx, FruitToolbarItem item)
        {
            ...
            {
                var ptg = _view.ptg;
                if (ptg == null || idx >= ptg.Length) return;

                var data  = new ToolbarView.SlotDrawData(item.Name, item.IconData);
                var slots = _view.m_slotsViews;
                var view  = (slots != null && idx < slots.Length) ? slots[idx] : null;

                if (ptg[idx] == null) ptg[idx] = new ToolbarView.ev(data, view);
                else                  ptg[idx].ptd = data;
            }
        ...
        internal static void RefreshDisplay(FruitToolbarItem item)
        {
            ...
            try
            {
                if (item.Descriptor != null)
                {
                    ...
                    item.IconData = item.Descriptor.tch;
                }
